...
#define SPLINE_PARAMS_NUM (FISHEYE_TANGENT_NUM + FISHEYE_POLY_ORDER - 1 + FISHEYE_AFFINE_NUM + 2)
...
#define ROOT_POLYNOMIAL_ORDER (2 * FISHEYE_POLY_ORDER)
  ...
  SplineCamera(const std::string& cameraName, int imageWidth, int imageHeight, double k2, double k3, double k4,
               double k5, double k6, double k7, double p1, double p2, double A11, double A12, double A22, double u0,
               double v0, int isFast);
...
template <typename T>
T SplineCamera::r(T k2, T k3, T k4, T k5, T k6, T k7, T theta) {
  // clang-format off
    return theta
        + k2 * theta * theta
...
template <typename T>
void SplineCamera::spaceToPlane(const T* const params, const T* const q, const T* const t,
  ...
  Eigen::Matrix<T, 2, 1> du = Eigen::Matrix<T, 2, 1>(T(2.0) * p1 * u * v + p2 * (r_sqr + T(2.0) * u * u),
