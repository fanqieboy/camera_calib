...
#define FISHEYE_PARAMS_NUM (FISHEYE_TANGENT_NUM + FISHEYE_POLY_ORDER - 1 + FISHEYE_AFFINE_NUM + 2)
...
class PolyFisheyeCamera : public Camera {
 public:
  class Parameters : public Camera::Parameters {
   public:
    Parameters();
    Parameters(const std::string& cameraName, int w, int h, double k2, double k3, double k4, double k5, double k6,
               double k7, double p1, double p2, double A11, double A12, double A22, double u0, double v0, int isFast);
...
#define ROOT_POLYNOMIAL_ORDER (2 * FISHEYE_POLY_ORDER)
   public:
    ...
    void backprojectSymmetric(const Eigen::Vector2d& p_u, double& cos_theta, double& sin_theta, double& cos_phi,
  ...
  class FastCalcTABLE {
   ...
   private:
    ...
    bool calcRToAngle(eigen_utils::Matrix& _rToAngle, const int _numDiffR, const double _diffR, const double _maxangle);
  ...
  PolyFisheyeCamera(const std::string& cameraName, int imageWidth, int imageHeight, double k2, double k3, double k4,
                    double k5, double k6, double k7, double p1, double p2, double A11, double A12, double A22,
                    double u0, double v0, int isFast);

  PolyFisheyeCamera(const Parameters& params);

  Camera::ModelType modelType() const;
  const std::string& cameraName() const;
  std::string& cameraName() override { return mParameters.cameraName(); }
...
template <typename T>
T PolyFisheyeCamera::r(T k2, T k3, T k4, T k5, T k6, T k7, T theta) {
  // clang-format off
    return theta
        + k2 * theta * theta
...
template <typename T>
void PolyFisheyeCamera::spaceToPlane(const T* const params, const T* const q, const T* const t,
  ...
  Eigen::Matrix<T, 2, 1> du = Eigen::Matrix<T, 2, 1>(T(2.0) * p1 * r_x * r_y + p2 * (r_sqr + T(2.0) * r_x * r_x),
