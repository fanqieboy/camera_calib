...
class FovCamera : public Camera {
 public:
  ...
  FovCamera(const std::string& cameraName, int imageWidth, int imageHeight, double omg, double fx, double fy, double u0,
            double v0);
...
template <typename T>
T FovCamera::rd(T omg, T ru) {
  return T(1) / (omg)*atan(T(2) * ru * tan(omg * 0.5));
}

template <typename T>
T FovCamera::rd_inverse(T omg, T rd) {
  return tan(rd * omg) / (T(2) * tan(omg * 0.5));
}

template <typename T>
T FovCamera::ru(const Eigen::Matrix<T, 3, 1>& P) {
  return sqrt((P(0) * P(0) + P(1) * P(1)) / (P(2) * P(2)));
}

template <typename T>
void FovCamera::spaceToPlane(const T* const params, const T* const q, const T* const t, const Eigen::Matrix<T, 3, 1>& P,
                             Eigen::Matrix<T, 2, 1>& p) {
  T P_w[3];
  P_w[0] = T(P(0));
  P_w[1] = T(P(1));
  ...
  Eigen::Matrix<T, 2, 1> p_u = Eigen::Matrix<T, 2, 1>(P(0) / P(2) * rd_p / ru_p, P(1) / P(2) * rd_p / ru_p);
