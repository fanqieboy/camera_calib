...
int main(int argc, char** argv) {
  ...
  if (is_first_run) {
    ...
    preprocess = new cv_utils::fisheye::PreProcess(cv::Size(image_src.cols,  //
                                                            image_src.rows),
                                                   cropper_size, cropper_center, resize_scale);
  ...
  for (image_index = 0; image_index < imageFilenames.size(); ++image_index) {
    ...
    aslam::cameras::GridCalibrationTargetAprilgrid aprilgrid(boardSize.height, boardSize.width, 0.075, 0.2);
    ...
    if (succ) {
      std::cerr << "# INFO: Detected chessboard in image " << image_index + 1 << ", " << imageFilenames.at(image_index)
                << std::endl;
      ...
      std::cout << "\033[31;47;1m"
                << "# INFO: Did not detect chessboard in image: " << imageFilenames.at(image_index) << "\033[0m"
                << std::endl;
    }
