...
FovCamera::FovCamera() : m_inv_K11(1.0), m_inv_K13(0.0), m_inv_K22(1.0), m_inv_K23(0.0) {}

FovCamera::FovCamera(const std::string& cameraName, int imageWidth, int imageHeight, double omg, double fx, double fy,
                     double u0, double v0)
    : mParameters(cameraName, imageWidth, imageHeight, omg, fx, fy, u0, v0) {}

FovCamera::FovCamera(const FovCamera::Parameters& params) : mParameters(params) {
  calcKinvese(params.fx(), params.fy(), params.u0(), params.v0());
}

Camera::ModelType FovCamera::modelType() const { return mParameters.modelType(); }

const std::string& FovCamera::cameraName() const { return mParameters.cameraName(); }

int FovCamera::imageWidth() const { return mParameters.imageWidth(); }

int FovCamera::imageHeight() const { return mParameters.imageHeight(); }

void FovCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p) const {
...
void FovCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p, float image_scalse) const {
 ...
 */
void FovCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p, Eigen::Matrix<double, 2, 3>& J) const {
  double ru_p = ru(P);
  double rd_p = rd(mParameters.omg(), ru_p);
...
void FovCamera::estimateIntrinsics(const cv::Size& boardSize,
...
void FovCamera::liftSphere(const Eigen::Vector2d& p, Eigen::Vector3d& P) const { liftProjective(p, P); }

void FovCamera::rayToPlane(const Ray& ray, Eigen::Vector2d& p) const {
  Eigen::Vector3d P(sin(ray.theta()) * cos(ray.phi()), sin(ray.theta()) * sin(ray.phi()), cos(ray.theta()));
...
void FovCamera::liftProjectiveToRay(const Eigen::Vector2d& p, Ray& ray) const {
...
void FovCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const {
  // Lift oints to normalised plane
  double theta, phi;
  // Obtain a projective ray

  Eigen::Vector2d p_u((p(0) - mParameters.u0()) / mParameters.fx(), (p(1) - mParameters.v0()) / mParameters.fy());
  double rd = p_u.norm();
...
void FovCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P, float image_scale) const {
...
void FovCamera::undistToPlane(const Eigen::Vector2d& p_u, Eigen::Vector2d& p) const {}

cv::Mat FovCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2, float fx, float fy, cv::Size imageSize,
...
int FovCamera::parameterCount() const { return FOV_PARAM_NUM; }

const FovCamera::Parameters& FovCamera::getParameters() const { return mParameters; }
...
void FovCamera::writeParametersToYamlFile(const std::string& filename) const { mParameters.writeToYamlFile(filename); }

std::string FovCamera::parametersToString() const {
...
bool FovCamera::calcKinvese(double fx, double fy, double u0, double v0) {
...
double FovCamera::getInv_K23() const { return m_inv_K23; }

double FovCamera::getInv_K22() const { return m_inv_K22; }

double FovCamera::getInv_K13() const { return m_inv_K13; }

double FovCamera::getInv_K11() const { return m_inv_K11; }

FovCamera::Parameters::Parameters() : Camera::Parameters(FOV), m_omg(0.0), m_fx(1.0), m_fy(1.0), m_u0(0.0), m_v0(0.0) {}

FovCamera::Parameters::Parameters(const std::string& cameraName, int w, int h, double omg, double fx, double fy,
                                  double u0, double v0)
    : Camera::Parameters(FOV, cameraName, w, h), m_omg(omg), m_fx(fx), m_fy(fy), m_u0(u0), m_v0(v0) {}

double& FovCamera::Parameters::omg() { return m_omg; }

double& FovCamera::Parameters::fx() { return m_fx; }

double& FovCamera::Parameters::fy() { return m_fy; }

double& FovCamera::Parameters::u0() { return m_u0; }

double& FovCamera::Parameters::v0() { return m_v0; }

double FovCamera::Parameters::omg() const { return m_omg; }

double FovCamera::Parameters::fx() const { return m_fx; }

double FovCamera::Parameters::fy() const { return m_fy; }

double FovCamera::Parameters::u0() const { return m_u0; }

double FovCamera::Parameters::v0() const { return m_v0; }
...
FovCamera::Parameters& FovCamera::Parameters::operator=(const FovCamera::Parameters& other) {
  if (this != &other) {
...
std::ostream& operator<<(std::ostream& out, const FovCamera::Parameters& params) {
